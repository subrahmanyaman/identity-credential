 ...
 */
package com.android.javacard.mdoc.provision;

import android.util.Log;

import com.android.javacard.mdoc.CBORBase;
import com.android.javacard.mdoc.CBORDecoder;
import com.android.javacard.mdoc.CBOREncoder;
import com.android.javacard.mdoc.MdlSpecifications;
import com.android.javacard.mdoc.SEProvider;
import com.android.javacard.mdoc.X509CertHandler;
...
public class ProvisioningApplet extends Applet implements ExtendedLength {
    ...
    private static void createDocument(MDoc doc, boolean testCred, byte[] scratch, short start,
                                       short len, byte[] docType, short docTypeStart,
                                       short docTypeLen) {
    ...
    private void handleSwapInMdoc(APDU apdu, byte slot, byte op, short start, short len) {
        ...
        if (op == BEGIN) {
            ...
            Log.d("ProvisioningApplet",  "Venkat debug1");
            // Update operation decrypts the data and stores it.
            short end = mSEProvider.encryptDecryptInPlace(buf, start, len, mScratch, (short)0,
                                                          (short)mScratch.length);
            Log.d("ProvisioningApplet",  "Venkat debug2");
            short size = (short)(end - start);
            byte[] apduBuf = apdu.getBuffer();
            Util.arrayCopyNonAtomic(buf, start, apduBuf, (short)0, size);
            Log.d("ProvisioningApplet",  "Venkat debug3");
            doc.store(apduBuf, (short)0, size);
            Log.d("ProvisioningApplet",  "Venkat debug4");
        } else {
            ISOException.throwIt(ISO7816.SW_INS_NOT_SUPPORTED);
        }
    }
    ...
    private void handleGetInformation(APDU apdu, byte slot) {
        byte[] buf = heap;
        if (slot < 0) {
            Util.arrayCopyNonAtomic(INFORMATION, (short)0, buf, (short)0,
    ...
    private void handleCreateAuthKeys(APDU apdu, byte slot, short notBeforeStart,
        ...
        short certLen = 0;
        if (mSEProvider.SIGNING_OPTION == SEProvider.SIGNING_OPTION_FK) {
            certLen = mSEProvider.generateSigningKeyCert(
                (ECPublicKey)(mSigningKey.getPublic()), (ECPrivateKey)(mAttestKey.getPrivate()),
                buf, notBeforeStart, notBeforeLen, buf, notAfterStart, notAfterLen, buf, certStart,
                SEProvider.SIGNING_CERT_MAX_SIZE, mScratch, (short)0, (short)(mScratch.length));
        } else {
