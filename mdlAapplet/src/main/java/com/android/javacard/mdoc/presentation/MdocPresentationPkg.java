...
public class MdocPresentationPkg {
    ...
    public void incrementUsageCount() { mUsageCount++; }
    ...
    public void write(byte[] buf, short start, short len) {
        Log.d("MdocPresentationPkg", "Venkat write debug1");
        if ((short)(mHeapIndex + len) > (short)(mHeap.length)) {
            ISOException.throwIt(ISO7816.SW_DATA_INVALID);
        }
        Log.d("MdocPresentationPkg", "Venkat write debug2 heapIdx:"+mHeapIndex + " len:"+len);
        Util.arrayCopyNonAtomic(buf, start, mHeap, mHeapIndex, len);
        Log.d("MdocPresentationPkg", "Venkat write debug3 heapIdx: "+(mHeapIndex + len));
        mHeapIndex += len;
    }
